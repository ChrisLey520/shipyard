...
- **Monitoring v2**：`apps/monitoring-server` 使用 **独立 PostgreSQL 库** 与 **独立生成 Prisma Client**（`src/generated/monitoring-prisma`，避免覆盖主应用 `@prisma/client`）；小时预聚合桶、事件保留任务、`GET /health`、Source map 上传与错误堆栈符号化、管理 API（项目创建 / 轮换 ingest token、按小时指标、告警规则）；企微 / 飞书 / 通用 Webhook 告警投递与基础 SSRF 校验。`apps/monitoring-web` 增加趋势条、详情符号化与「项目与告警」页。`packages/monitoring-sdk` 支持可选 **IndexedDB 持久化队列**（`persistQueue`）。
