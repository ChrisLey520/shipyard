...
- `apps/monitoring-server`、`apps/monitoring-web`、`packages/monitoring-sdk`、`packages/monitoring-contracts`：与 Shipyard 主业务 **数据不绑定** 的监控接收端、管理台与客户端 SDK。
- **下一版（v2）规划**：[需求规格](.cursor/plans/monitoring-v2-需求规格.md) · [路线图](.cursor/plans/monitoring-v2-路线图.plan.md)；Phase A 基线说明见 [三端方案](.cursor/plans/monitoring-sdk-三端方案.plan.md)。
