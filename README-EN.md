...
|------------|---------------|-------|
| `local` executor | Writable target path on the machine running the Deploy Worker; no SSH server required | Reuses the same server-deploy strategy semantics as SSH (`direct`/single-host `rolling`, Linux+domain blue-green, Linux canary, PM2/Nginx/hooks); commands and rsync run on the Worker machine instead of through SSH. If the Worker runs inside a Kubernetes Pod, “local” means the Pod filesystem; mount a `hostPath`/PVC into the Worker to write a node host path. |
| `direct` / `rolling`, multi-host | SSH; `EnvironmentServer` rows | Serial rsync by `sortOrder`; `primaryServerId` or first host owns Nginx/domain. The local executor has no extra hosts, so `rolling` behaves as a single-host sync. |
| `blue_green` (static) | Linux + domain | Slot dirs `.shipyard-bg0` / `.shipyard-bg1`, switch Nginx root; failed health rolls root back |
| `blue_green` (SSR) | Linux + domain | Dual slot dirs `.shipyard-bg0`/`1`, stable local ports, PM2 names `sh-env-<slug>-<env>-bg*`, and Nginx reverse-proxy switching; old slot is removed only after external health and Prometheus gates pass. On multi-host SSH envs, only the primary host performs the switch. |
| `canary` | SSH or local; Linux primary/Worker; `nginxCanaryPath` | **split_clients** (default): upstream names + `canaryPercent`, `proxy_pass http://$shipyard_canary_pool;`. **upstream_weight**: `nginxCanaryTemplate`, `nginxCanaryUpstreamName`, stable/candidate `host:port` backends + percent weights. **Manual** `nginxCanaryBody` wins. Backup on `nginx -t` failure. See [docs/runbooks/canary-nginx.md](docs/runbooks/canary-nginx.md) |
| Prometheus gate | `gates.prometheus.queryUrl` | GET + JSON parse; same SSRF rules as notification outbound |
| pre/post hooks | SSH / local | SSH runs on the primary host; local deploy runs under the Worker machine's `deployPath` with `bash -lc` (uses `timeout 120` when available) |
| Kubernetes | Image push enabled; Worker can run `kubectl` | `kubectl set image` + `rollout status`; optional `rolloutTimeoutSeconds` (default 600s); with `strategy: rolling`, optional `rollingUpdateMaxSurge` / `rollingUpdateMaxUnavailable` patched before set image. kubeconfig can come from an **organization registered cluster** (`clusterId`) or the **local Worker kubeconfig** (`KUBECONFIG` / `~/.kube/config`, optional `kubeconfigPath`) when the Worker runs directly on the Ubuntu host. **Not** `canary` or `blue_green` (use SSH/local server deploy). GitOps may overwrite patches — see repo `.cursor/plans/shipyard-顺架构发布策略-需求规格.md`. |
